...
import { clearCart } from "@/src/lib/utils/cartUtils";
import {
  getLastOrder,
  getEstimatedDelivery,
  mapStrapiOrderToLastOrder,
} from "@/src/lib/utils/lastOrder";
import OrderInvoice from "@/src/components/common/OrderInvoice";

function OrderCompletedHero() {
  return (
    <div
      className="relative py-20 md:py-24 bg-cover bg-center bg-no-repeat no-print"
...
async function fetchOrderFromApi(orderId, email) {
  ...
  return mapStrapiOrderToLastOrder(json.data);
}
...
function OrderCompletedContent() {
  ...
  const loadOrder = useCallback(async () => {
    ...
    if (orderId && email) {
      const fromApi = await fetchOrderFromApi(orderId, email);
      if (fromApi?.orderId) {
        setOrder(fromApi);
        clearCart();
        return;
      ...
      setLoadError(
        "We could not load this order. Check your confirmation email or try My Orders."
      );
      return;
    }
  ...
  if (!ready) {
    return (
      <div className="min-h-screen bg-gray-50 pt-20 flex items-center justify-center no-print">
  ...
  if (!order?.orderId) {
    return (
      <div className="min-h-screen bg-gray-50 pt-20 no-print">
        <OrderCompletedHero />
          ...
          <Container>
            <div className="max-w-lg mx-auto text-center bg-white rounded-2xl shadow-lg p-10">
              ...
              <p className="text-gray-600 mb-6">
                {loadError ||
                  "Complete checkout and payment to see your order here, or open the link from your confirmation email."}
              </p>
  ...
  const displayOrderId = String(order.orderId).startsWith("#")
    ? order.orderId
    : `#${order.orderId}`;
  ...
  return (
    <>
      ...
      <OrderInvoice order={order} estimatedDelivery={estimatedDelivery} />
        ...
        <Section className="bg-gray-50">
          <Container>
            <div className="max-w-4xl mx-auto">
              ...
              <div className="bg-yellow-500 rounded-2xl p-6 md:p-8 mb-12">
