...
export default function EditPlantPage() {
  ...
  const [newImages, setNewImages] = useState([]); // { id, url } from upload
  ...
  const fetchPlant = async () => {
    try {
      ...
      const response = await adminAction.getPlantById(id, token);
      ...
      const imgs = (attrs.images?.data || []).map((img) => ({
        id: img.id,
        url: img.attributes?.url?.startsWith("http")
          ? img.attributes.url
          : `${STRAPI_BASE_URL}${img.attributes?.url || ""}`,
      }));

      setExistingImages(imgs);
      const hasVal = (v) => v != null && String(v).trim() !== "";
      const anySize = hasVal(attrs.priceSmall) || hasVal(attrs.priceMedium) || hasVal(attrs.priceLarge);
      const legacyPrice = hasVal(attrs.price);
      const categoryId = attrs.categories?.data?.[0]?.id ?? attrs.categories?.[0];
      setFormData({
        title: attrs.title || "",
        ...
        rating: attrs.rating ?? "",
        stock: attrs.stock ?? "",
        enableSmall: hasVal(attrs.priceSmall),
        enableMedium: hasVal(attrs.priceMedium) || (!anySize && legacyPrice),
        enableLarge: hasVal(attrs.priceLarge),
        priceSmall: attrs.priceSmall ?? "",
        priceMedium: hasVal(attrs.priceMedium) ? (attrs.priceMedium ?? "") : (legacyPrice ? String(attrs.price) : ""),
        priceLarge: attrs.priceLarge ?? "",
        discountPercent: attrs.discountPercent ?? "",
        offerStart: attrs.offerStart || "",
        offerEnd: attrs.offerEnd || "",
      });
    } catch (error) {
      console.error("Error fetching plant:", error);
      toast.error(error.response?.data?.error?.message || "Failed to load plant");
      router.push("/admin/plants");
    } finally {
      setFetching(false);
    }
  ...
  const buildPayload = () => {
    ...
    if (allIds.length) base.images = allIds;
    return base;
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await adminAction.updatePlant(id, data, token);
      toast.success("Plant updated successfully!");
      router.push("/admin/plants");
    } catch (error) {
      console.error("Error updating plant:", error);
      toast.error(error.response?.data?.error?.message || "Failed to update plant");
      setLoading(false);
    }
  };
              ...
              <div className="space-y-2">
                ...
                >
                  ...
                  {categories.map((c) => {
                    ...
                    const label = attrs.name || attrs.title || attrs.slug || c.name || c.slug || `Category #${c.id}`;
          ...
          <CardHeader>
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <CardDescription>Upload or remove plant images.</CardDescription>
              </div>
            </div>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            {allImages.length > 0 && (
              <div className="flex flex-wrap gap-3">
                {existingImages.map((f) => (
                  <div key={`e-${f.id}`} className="relative w-24 h-24 rounded-lg overflow-hidden border bg-gray-100">
                ...
                {newImages.map((f) => (
                  <div key={`n-${f.id}`} className="relative w-24 h-24 rounded-lg overflow-hidden border bg-gray-100">
                    <Image src={f.url} alt="" fill className="object-cover" />
                    <button
                      type="button"
